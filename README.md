...
2. `grok-code-agent-0.2.0.vsix`を選択します。
...
`store: false`のまま複数ターンを継続するため、xAIから返された最新の暗号化済み推論状態も会話履歴と一緒にローカル保存します。この状態は表示用Webviewには渡さず、次のxAI APIリクエストにだけ使用します。

- `Enter`で送信、`Shift+Enter`で改行
- 応答中の停止、新しいチャット、履歴の切り替えと削除
- 思考中のスピナー表示
- APIキーで利用可能なGrokテキストモデルの選択
- チャットごとの追加指示（回答スタイルや前提条件）の保存
- 入力欄の`＋`からAPIキー、VS Code Agent、拡張機能設定へアクセス
...
- 専用チャットの複数ターンに必要な最新の暗号化済み推論状態は、表示用の会話履歴とは分離して拡張機能ストレージへ保存します。APIキー変更時に破棄します。
